...
`redo/inputs/g0_measurements.csv`, generated by `redo/build/g0_measure_axes.py`.

### The population reproduces the spec exactly

| | measured | `GROUP0_SYSTEMS` says |
|---|---:|---:|
| calibration, Active/Inactive | **726** | 726 |
| application, Active/Inactive | **610** | 610 |
| Intermediate | **21** | 21 |

So `g0_calibration_set.py`'s enumeration and the measurement agree on what the
object is. That was the thing most worth checking first and it is clean.

### The axis-definable count does NOT, and the shortfall is informative

§0.1 / §5 F0b predicted **514** calibration structures would survive the
"NPxxY axis defined" gate. The measured chain:

| step | n | what removes it |
|---|---:|---|
| calibration Active/Inactive | 726 | — |
| sequence table says 5.58 or 7.53 is not Tyr | **516** | Q0b, the declared gate — **against 514 predicted, 0.4% off** |
| coordinates disagree with the sequence table | **501** | **15 structures — Q6** |
| anchor unmodelled / SIFTS / OH absent | **492** | structural, not gated anywhere |

**§0.1's prediction was right about the thing it predicted.** It is a *sequence*
gate and it lands within two structures of the table. The further 24 are losses
it never claimed to cover, and separating them is the whole point — reporting
"514 predicted, 492 measured" as a miss would be the compare-like-with-like
failure again.

**The 15 are the interesting ones.** The reference sequence says tyrosine at both
anchors and the deposited coordinates do not confirm it — engineered
substitutions and unmodelled anchors in the construct, across nine receptors
(5HT4R ×3, P2Y12 ×2, CCR6 ×2, CXCR3 ×2, DRD4 ×2, ADA1A, GASR, TSHR, E7FEL0).
**§5's Q6 is recorded as "engineered-substitution cap: declared, not yet
enforceable". It is now measured: 15 of 516, 2.9%.**

### Two things the pass itself taught

- **The identity ladder carries more than SIFTS does.** 605 measurements came
  from identity numbering against 426 from SIFTS. §3.3 described SIFTS-first with
  identity as the fallback; the fallback is doing most of the work, and every row
  records which strategy produced it.
- **Q7's outlier screen matters far more than the pilot suggested.** The pilot
  found one structure above 20 Å on NPxxY (`6RNK`, 25.59). The full pass finds
  **39**. Q7 was declared in advance, which is the only reason this is a screen
  and not a decision made after seeing the plot.

### The separation, descriptive only

**No threshold is fitted here. `GROUP0_SYSTEMS` §6: "This section must be settled
before any threshold is fitted", and D4 is OPEN (`ASKS.md:106`).**

| set | state | n | NPxxY-OH median (IQR) | tilt median (IQR) |
|---|---|---:|---|---|
| calibration | Active | 406 | 4.22 (3.81–4.84) | 17.96 (17.32–18.55) |
| calibration | Inactive | 86 | 11.44 (10.04–14.27) | 12.23 (11.69–12.47) |
| calibration | Intermediate | 11 | 15.78 (15.51–22.08) | 14.92 (12.06–15.46) |
| application | Active | 321 | 4.40 (4.00–4.80) | 17.89 (17.50–18.79) |
| application | Inactive | 208 | 10.86 (9.76–13.88) | 12.07 (11.89–12.47) |

The IQRs do not overlap on either axis in either set. **Two observations to carry
into the fit and not to act on now**: the calibration Inactive NPxxY *minimum* is
8.95, below the applied 9.08 cut; and the calibration Active tilt *minimum* is
12.11, below the applied 14.932. Both cuts already have at least one off-panel
counterexample. What that means is a question for after D4.

**Only 11 of the 21 Intermediates are measurable**, which bears on E0.3's
pre-commitment — it was written as "if ≥15 of 21 fall strictly between the two
cuts". At n=11 that rule cannot be satisfied as written and needs restating
before the numbers are looked at.

### A correction to F-22's framing, and to my own commit message

I added `OUT_DEFAULT` to `g0_measure_axes.py` naming the canonical output, and
said in the commit that this "makes the attribution true rather than cosmetic".
**It does not, and the file landed `unattributed` anyway.** `manifest.py` requires
the *bound name* to reach `open()`; this script writes through `a.out`, an
argparse attribute, so the static walk cannot see it.

Checking how much of the backlog has that shape: **exactly one file of the 15**.
The other 14 have **no candidate writer in `redo/build/` at all** — they are not
unattributable-by-design, they are orphaned, which is the harder half of F-22 and
is unchanged. The `OUT_DEFAULT` change is still worth keeping for the canonical
path and the truncation guard it made necessary, but not for the reason I gave.
