...
# Backward-compat re-exports — pocket metrics landed in scorer/pocket_metrics.py
# (Block C Gate 0.1, 2026-09-03). Every downstream caller that imports the
# four pocket-axis functions from ``scorer.axes`` keeps working; the
# implementation lives in the sibling module. The bundle entry point
# used by the orchestrator is imported here for the same reason.
from scorer.pocket_metrics import (  # noqa: F401 — re-export
    POCKET_BW_LABELS,
...
# Post-run receipts landed as a sibling module in Block C Stage 0 Step 1.4
# (2026-09-04). Re-exported here for the same reason pocket_metrics is —
# any downstream caller that imports from ``scorer.axes`` keeps working.
from scorer.post_run_receipts import (  # noqa: F401 — re-export
    check_ligand_present,
    check_receptor_slug_populated,
    count_ligand_heavy_atoms,
)
