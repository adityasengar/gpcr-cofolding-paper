...
# MSA source — pre-fetched .a3m via JSON `unpairedMsaPath`/`pairedMsaPath`
# (Tier D3 depth-tier mode) vs Protenix's own MSA server (default; every
# other tier). When MSA_A3M_PATH is set, scorer/propose.py has already
# populated the JSON paths, so the launcher drops --msa_server_mode.
PROTENIX_MSA_ARGS=(--msa_server_mode protenix)
if [ -n "${MSA_A3M_PATH:-}" ]; then
    PROTENIX_MSA_ARGS=()
    echo "MSA source  : pre-fetched (MSA_A3M_PATH=$MSA_A3M_PATH; --msa_server_mode disabled)"
else
    echo "MSA source  : Protenix MSA server (--msa_server_mode protenix)"
fi
