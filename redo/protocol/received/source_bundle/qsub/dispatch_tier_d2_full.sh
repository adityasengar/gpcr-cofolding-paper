#!/bin/bash
#
# qsub/dispatch_tier_d2_full.sh — Tier D2 full pool driver.
#
...
# unchanged, pointed at the D2 full pool via $BLOCK_B_POOL env.
#
...
# ~2 h; D2 is 2,400 preds so ~2/14 * 2h * 19/12 = ~0.5 h theoretical,
# reserve 2 h for overhead + slow-cell drift.
#
# Prerequisite: pool has queue.csv + queue_state.csv built from
# experiments/023_tier_d2_directed_inactive/manifest/tier_d2_manifest.dispatch.csv
# via scripts/build_block_b_campaign_queue.py.
#
# Usage (on basel-hpc login node):
#   bash qsub/dispatch_tier_d2_full.sh
#     (defaults: N_H100=12, POOL_NAME=tier_d2_full_2026_09_07)

set -o pipefail

POOL_NAME="${POOL_NAME:-tier_d2_full_2026_09_07}"
TIER_D2_POOL="${TIER_D2_POOL:-/hpc/scratch/sengaad1/paper_af3/experiments/023_tier_d2_directed_inactive/full/pool}"
...
log() { echo "[$(date -u +%FT%TZ) tier-d2-full-launcher] $*"; }

# 1. Pool prereqs
for f in "$TIER_D2_POOL/queue.csv" "$TIER_D2_POOL/queue_state.csv"; do
    if [ ! -f "$f" ]; then
        log "ABORT: missing $f"
        log "  Build the queue first from the dispatch manifest:"
        log "    python3 $REPO/scripts/build_block_b_campaign_queue.py \\"
        log "      --manifest $REPO/experiments/023_tier_d2_directed_inactive/manifest/tier_d2_manifest.dispatch.csv \\"
        log "      --pool $TIER_D2_POOL"
        exit 2
    fi
done
mkdir -p "$TIER_D2_POOL/logs"
...
log "watch queue drain: watch \"awk -F, 'NR>1{print \\\$3}' $TIER_D2_POOL/queue_state.csv | sort | uniq -c\""
log ""
log "Expected wall: ~1 h for 240 rows × 10 samples = 2,400 predictions on $N_H100 workers"
log "When drain completes, rescore with:"
log "    python3 $REPO/scripts/rescore_parallel.py --pool $TIER_D2_POOL --out $REPO/experiments/023_tier_d2_directed_inactive/analysis/full"
