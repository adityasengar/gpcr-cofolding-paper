...
def cmd_pop(args) -> int:
    ...
    # Tier D3 depth-tier plumbing: optional per-row overrides. Empty when
    # the manifest omits the column — preserves default (live-server MSA).
    msa_a3m = q.get("msa_a3m_path", "").strip()
    if msa_a3m:
        print(f"export MSA_A3M_PATH={msa_a3m!r}")
    chai_msa_dir = q.get("chai_msa_directory", "").strip()
    if chai_msa_dir:
        print(f"export CHAI_MSA_DIRECTORY={chai_msa_dir!r}")
    return 0
