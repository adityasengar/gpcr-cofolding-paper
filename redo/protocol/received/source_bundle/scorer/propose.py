...
def _boltz_msa_field(msa_a3m_path: str | None, chain_id: str) -> str:
    """Return the ``msa: <path>`` YAML line to embed inside a Boltz protein
    chain entry. Empty string when ``msa_a3m_path`` is None or empty — that
    preserves the pre-Tier-D3 default of live ColabFold-server fetching.

    ``msa_a3m_path`` may be a plain string (single .a3m applied to every
    protein chain) or a mapping ``{chain_id: path}`` for per-chain paths in
    multi-chain complexes (D3 multi-chain support, currently unused).
    """
    if not msa_a3m_path:
        return ""
    if isinstance(msa_a3m_path, dict):
        path = msa_a3m_path.get(chain_id)
        if not path:
            return ""
    else:
        path = str(msa_a3m_path)
    return f"      msa: {path}\n"


def _boltz_yaml_monomer(rseq: str, msa_a3m_path: str | None = None) -> str:
    return (
        "version: 1\n"
        "sequences:\n"
        "  - protein:\n"
        "      id: A\n"
        f"      sequence: {rseq}\n"
        + _boltz_msa_field(msa_a3m_path, "A")
    )


def _boltz_yaml_two_chain(rseq: str, pseq: str,
                          msa_a3m_path: str | None = None) -> str:
    return (
        "version: 1\n"
        "sequences:\n"
        "  - protein:\n"
        ...
        "      id: B\n"
        f"      sequence: {pseq}\n"
        + _boltz_msa_field(msa_a3m_path, "B")
    )
...
def _of3_apply_msa_paths(chains: list[dict[str, Any]],
                         msa_a3m_path: str | None) -> None:
    """Populate ``main_msa_file_paths`` + ``paired_msa_file_paths`` on each
    PROTEIN chain in ``chains`` when ``msa_a3m_path`` is set. Mutates in place.

    Per ``openfold3/projects/of3_all_atom/config/inference_query_format.py:60-66``,
    both fields are lists of absolute .a3m paths (or a directory containing
    ``.a3m`` files with basenames OF3 recognises — see the
    ``max_seq_counts`` registry in
    ``openfold3/projects/of3_all_atom/config/dataset_config_components.py``,
    e.g. ``colabfold_main``, ``bfd_uniref_hits``).

    Empty / None value → no change (default is live ColabFold-server fetch
    on the launcher CLI).

    D3 convention: the D3 MSA cache stores each subsampled .a3m as
    ``<cache>/{full|seed_S/depth_D}/<receptor>/colabfold_main.a3m`` so
    OF3's registry-based filter accepts it. If the caller passes the
    receptor-directory path, we forward it verbatim; if they pass a file
    path (e.g. ``/path/.../aa2ar/colabfold_main.a3m``), we forward the
    parent directory so OF3 sees the enclosing dir.
    """
    if not msa_a3m_path:
        return
    from pathlib import Path
    is_dict = isinstance(msa_a3m_path, dict)
    for chain in chains:
        if chain.get("molecule_type", "").upper() != "PROTEIN":
            continue
        chain_ids = chain.get("chain_ids") or []
        cid = chain_ids[0] if chain_ids else ""
        raw = msa_a3m_path.get(cid) if is_dict else str(msa_a3m_path)
        if not raw:
            continue
        p = Path(raw)
        # OF3 accepts a directory of .a3m files (basenames must be in the
        # max_seq_counts registry). Pass the directory so OF3 auto-scans;
        # if the path is a file, use its parent so a `colabfold_main.a3m`
        # inside is recognised.
        of3_path = str(p.parent) if p.suffix == ".a3m" else str(p)
        chain["main_msa_file_paths"] = [of3_path]
        chain["paired_msa_file_paths"] = [of3_path]


def _of3_query_dict(name: str, chains: list[dict[str, Any]], is_multimer: bool, seed: int,
                    msa_a3m_path: str | None = None) -> dict[str, Any]:
    _of3_apply_msa_paths(chains, msa_a3m_path)
    return {
        "seeds": [seed],
        "queries": {
            name: {
...
def _of3_json_monomer(name: str, rseq: str, seed: int,
                      ligand_chain: dict[str, Any] | None = None,
                      msa_a3m_path: str | None = None) -> str:
    ...
    return json.dumps(_of3_query_dict(name, chains, is_multimer=is_multimer,
                                      seed=seed, msa_a3m_path=msa_a3m_path),
                      indent=2) + "\n"
...
def _of3_json_two_chain(name: str, rseq: str, pseq: str, seed: int,
    ...
    if ligand_chain is not None:
        chains.append(ligand_chain)
    return json.dumps(_of3_query_dict(name, chains, is_multimer=True,
                                      seed=seed, msa_a3m_path=msa_a3m_path),
                      indent=2) + "\n"
...
def _protenix_apply_msa_paths(seqs: list[dict[str, Any]],
                              msa_a3m_path: str | None) -> None:
    """Populate ``unpairedMsaPath`` + ``pairedMsaPath`` on each proteinChain
    when ``msa_a3m_path`` is set. Mutates in place.

    Per ``protenix/data/msa/msa_featurizer.py:594-635``, both are absolute
    paths to `.a3m` files. Empty / None → no change (Protenix launcher's
    ``--msa_server_mode protenix`` default handles live fetch).

    ``msa_a3m_path`` accepts a plain string (all protein chains share the
    same path) or a mapping ``{chain_index: path}`` where index is the
    protein-chain position (0-based among proteinChain entries).
    """
    if not msa_a3m_path:
        return
    is_dict = isinstance(msa_a3m_path, dict)
    protein_idx = 0
    for entry in seqs:
        pc = entry.get("proteinChain")
        if pc is None:
            continue
        path = msa_a3m_path.get(protein_idx) if is_dict else str(msa_a3m_path)
        protein_idx += 1
        if not path:
            continue
...
def _receptor_nb_content(
    ...
    """Emit backbone-input file content for a (receptor + nanobody) row.

    Tier D2 (PREREG §D-2): nanobody-directed inactive/active-state arms.
    The Nb replaces Gα as the state-directing partner. Receptor lands on
    chain A; Nb lands on chain B (identical schema to cognate-Gα arms so
    the corpus format is preserved and no fold-model input-schema branch
    is introduced).

    Purely reuses the existing two-chain templaters; existing apo /
    cognate-Gα code paths are untouched. Introduced as a NEW branch, not
    a modification of ``_row_input_content``.
    """
    if backbone == "boltz":
        return _boltz_yaml_two_chain(receptor_seq, nb_seq)
    if backbone == "of3":
        return _of3_json_two_chain(input_name, receptor_seq, nb_seq, seed,
                                   ligand_chain=None)
    if backbone == "protenix":
        return _protenix_json_two_chain(input_name, receptor_seq, nb_seq,
                                        ligand_entry=None)
    if backbone == "chai":
        return _chai_fasta_two_chain("receptor", receptor_seq,
                                     "nb", nb_seq, ligand_fasta="")
    raise ValueError(f"unknown backbone {backbone!r} (tier D2 Nb path)")
