...
# Bug #4 fix (2026-09-07): also require the matched atoms to cover a
# MAJORITY of both the pred and the ref ligand — otherwise SMILES-derived
# atom names (Boltz-2, Chai-1, OpenDDE ``l01``) that coincidentally
# overlap with ≥5 CCD-canonical names on the ref side push through the
# fast path with a spurious "identity" match that is not chemically
# meaningful. When coverage on either side falls below this threshold,
# the row falls through to MCS. Empirically 0.8 (80%) is high enough to
# rule out the coincidental-5 case while still accepting sample-level
# variance where 1-2 atoms are named differently between pred and ref.
MCS_FALLBACK_MIN_COVERAGE: float = 0.8
...
def _mcs_ligand_rmsd(
    ...
    try:
        mcs = rdFMCS.FindMCS(
            ...
            # bonds from MolFromPDBBlock with proximityBonding are all
            # single, so bondCompare=CompareOrder was vacuous. Use
            # CompareAny so the MCS is driven by element+topology, not
            # unreliable bond-order labels.
    ...
    # Bug #3 fix (2026-09-07): enumerate ALL valid substructure matches on
    # both sides via GetSubstructMatches(uniquify=False) and pick the
    # (pred_match, ref_match) pairing that minimizes RMSD. The previous
    # implementation used GetSubstructMatch (singular, arbitrary mapping)
    # on both molecules independently, so any symmetry in the MCS query
    # produced a pairing whose RMSD depended on atom-order-in-file, not
    # on geometry. Reviewer test: 57% of true docks were miscalled as
    # failures under random atom orderings; direction of error was both
    # inflating and deflating.
    #
    # Cap match count to avoid pathological O(N!) blowups (dense symmetric
    # queries). 256 is chosen empirically to bound worst-case runtime
    # while covering all realistic ligand automorphism groups.
    _MAX_MATCHES = 256
    pred_matches = pred_mol.GetSubstructMatches(q, uniquify=False, maxMatches=_MAX_MATCHES)
    ref_matches = ref_mol.GetSubstructMatches(q, uniquify=False, maxMatches=_MAX_MATCHES)
    if not pred_matches or not ref_matches:
        return NaN, "mcs_substruct_mismatch", "no_match"
    q_n = q.GetNumAtoms()
    if any(len(m) != q_n for m in pred_matches) or any(len(m) != q_n for m in ref_matches):
        return NaN, "mcs_substruct_mismatch", "no_match"

    pred_conf = pred_mol.GetConformer()
    ref_conf = ref_mol.GetConformer()

    # Extract pred + ref positions once per match, then RMSD minimize.
    def _positions(conf, match):
        out = []
        for idx in match:
    ...
    pred_pos_variants = [_positions(pred_conf, m) for m in pred_matches]
    ref_pos_variants = [_positions(ref_conf, m) for m in ref_matches]

    best_sq = float("inf")
    best_n = 0
    for pv in pred_pos_variants:
        for rv in ref_pos_variants:
            sq = 0.0
            for (px, py, pz), (rx, ry, rz) in zip(pv, rv):
                sq += (px - rx) ** 2 + (py - ry) ** 2 + (pz - rz) ** 2
            if sq < best_sq:
                best_sq = sq
                best_n = len(pv)

    if best_n == 0 or not math.isfinite(best_sq):
        return NaN, "mcs_no_valid_pairing", "no_match"

    rmsd = math.sqrt(best_sq / best_n)
    note = (
        f"mcs_matched_{best_n}_of_pred_{pred_mol.GetNumAtoms()}"
        f"_ref_{ref_mol.GetNumAtoms()}"
        f"_variants_p{len(pred_pos_variants)}_r{len(ref_pos_variants)}"
    )
...
def ligand_rmsd_to_ref(
    ...
    **Consumers must filter decoy_lig rows** — the pose distance
    ...
    pred_atoms: list[tuple[str, gemmi.Position, str]] = []
    model = struct[0]
    # Pre-compute STD_AA residue count per chain so we can distinguish a
    # short peptide ligand (kept) from a G-protein heterotrimer subunit
    # (filtered). G-alpha ≈ 350aa, G-beta ≈ 340aa, G-gamma ≈ 70aa,
    # mini-G-alpha ≈ 250aa. Peptide ligands are typically 5-25aa.
    # Threshold 50aa excludes all four G-protein subunit types while
    # preserving peptide-ligand carve-outs.
    #
    # 2026-09-07 fix: prior code only filtered STD_AA on the receptor
    # chain, letting G-protein-subunit amino acids into the ligand-atom
    # pool on cognate-arm co-fold predictions. MCS then found spurious
    # substructure matches on Gα, producing garbage ligand_rmsd_to_ref
    # values (e.g. 96 Å on DRD2 full_agonist × cognate × Chai).
    _chain_std_aa_count: dict[str, int] = {}
    for _ch in model:
        _n = 0
        for _r in _ch:
            if one_letter(_r.name) in STD_AA:
                _n += 1
        _chain_std_aa_count[_ch.name] = _n
    _GPROTEIN_LEN_THRESHOLD = 50

    for ch in model:
        # Peptide ligands may land on a non-receptor chain (short — <50aa);
        # small molecules almost always land on the receptor chain as HETATM.
        # Scan every chain; skip STD_AA residues on the receptor chain OR
        # on any chain long enough to be a G-protein heterotrimer subunit.
        chain_is_gprotein_length = (
            _chain_std_aa_count.get(ch.name, 0) >= _GPROTEIN_LEN_THRESHOLD
        )
        for r in ch:
            # Skip receptor amino-acid residues on the receptor chain, and
            # skip amino-acid residues on any long non-receptor chain (i.e.,
            # Gα/Gβ/Gγ heterotrimer subunits under a cognate co-fold).
            if one_letter(r.name) in STD_AA:
                if ch.name == receptor_chain_name or chain_is_gprotein_length:
                    continue
    ...
    # Bug #4 guard: fast path also requires MAJORITY coverage on BOTH sides,
    # not just >= MIN_MATCHED. Without this, a SMILES-derived pred (Boltz /
    # Chai / OpenDDE l01) with 20 atoms that coincidentally shares 5 atom
    # names with a 25-atom CCD ref would go through the fast path with a
    # 5-atom match — chemically meaningless. Coverage guard forces such
    # rows to the MCS fallback where atom_pairing is validated by
    # substructure.
    pred_cov = len(matched_pred) / max(1, len(pred_by_name))
    ref_cov = len(matched_pred) / max(1, len(ref_by_name))
    if (len(matched_pred) >= MCS_FALLBACK_MIN_MATCHED
            and pred_cov >= MCS_FALLBACK_MIN_COVERAGE
            and ref_cov >= MCS_FALLBACK_MIN_COVERAGE):
        sq = 0.0
        for p, r in zip(matched_pred, matched_ref):
            sq += p.dist(r) ** 2
        ...
        note = (
            ...
            f"_covp{pred_cov:.2f}_covr{ref_cov:.2f}"
        )
...
def build_pocket_reference_cache(
    ...
    if struct is not None and len(struct) > 0:
        # Precompute per-chain STD_AA Cα positions to enable the pocket-proximity
        # tripwire — see Bug #2 (2026-09-07): scorer picks the LARGEST HETATM in
        # the reference by heavy-atom count, without checking that it's near
        # the receptor's binding pocket. Consequence on cognate co-fold
        # references: GDP (28 heavy atoms on Gα) beats the target agonist
        # (22 atoms in the pocket) → 68 Å spurious RMSD.
        #
        # Tripwire: only keep candidates whose centroid is within 8 Å of any
        # Cα on a receptor-plausible chain (200 ≤ chain STD_AA count ≤ 400).
        # 8 Å covers atoms sitting inside the pocket + reasonable slack for
        # sidechain reach. Chains outside 200-400 aa (Gα ≈ 350; Gβ ≈ 340;
        # Gγ ≈ 70; mini-Gα ≈ 250) mostly excluded — note Gα falls IN this
        # window on some refs, so proximity is what discriminates.
        _POCKET_CENTROID_MAX_DIST = 8.0
        _RECEPTOR_CHAIN_MIN_AA = 200
        _RECEPTOR_CHAIN_MAX_AA = 400
        _receptor_chain_ca: list[gemmi.Position] = []
        for ch in struct[0]:
            aa_count = sum(1 for r in ch if one_letter(r.name) in STD_AA)
            if not (_RECEPTOR_CHAIN_MIN_AA <= aa_count <= _RECEPTOR_CHAIN_MAX_AA):
                continue
            for r in ch:
                if one_letter(r.name) not in STD_AA:
                    continue
                ca = r.find_atom("CA", "\0")
                if ca is not None:
                    _receptor_chain_ca.append(ca.pos)

        # Group HETATM residues by (chain, resname, seqid). Two candidate
        # lists: pocket-proximal (tripwire pass) + fallback (all HETATMs).
        # We prefer pocket-proximal; fall back to the legacy largest-HETATM
        # behavior only if the tripwire rejects everything.
        candidates_all: list[tuple[int, str, list, float]] = []
        for ch in struct[0]:
            for r in ch:
                if r.name in _NON_LIGAND_RESNAMES:
                    continue
                ...
                if not heavy:
                    continue
                # Centroid distance to nearest receptor-chain Cα
                cx = sum(a.pos.x for a in heavy) / len(heavy)
                cy = sum(a.pos.y for a in heavy) / len(heavy)
                cz = sum(a.pos.z for a in heavy) / len(heavy)
                min_d = float("inf")
                for ca in _receptor_chain_ca:
                    d = math.sqrt(
                        (ca.x - cx) ** 2 + (ca.y - cy) ** 2 + (ca.z - cz) ** 2
                    )
                    if d < min_d:
                        min_d = d
                candidates_all.append((len(heavy), r.name, heavy, min_d))

        # Tripwire-passing candidates only.
        candidates_pocket = [
            c for c in candidates_all
            if c[3] <= _POCKET_CENTROID_MAX_DIST
        ]
        pick_pool = candidates_pocket if candidates_pocket else candidates_all

        if pick_pool:
            # Pick largest by heavy-atom count, resname as tie-break.
            pick_pool.sort(key=lambda c: (c[0], c[1]), reverse=True)
            _, ligand_resname, heavy_atoms, _picked_dist = pick_pool[0]
