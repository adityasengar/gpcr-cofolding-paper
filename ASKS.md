...
### A2. The adaptation band — **TIME-SENSITIVE, worthless if taken late**
...
### A3. The measurement pass — the largest single dependency
...
### A4. The title, and whether the ladder runs
...
### A5. Group 2 — may the antagonist level be heterogeneous?
`GROUP2_LIGANDS.md` §6. Restricting it costs **3 clusters**, MDE 0.314 → 0.352.

### A6. Group 0's three open items
...
### A7. The `paper_af3` co-authorship conversation — **do this before anything is written**
...
### C1. `corpus_check.sh` classifies the four deliberately-unread `refs.bib` entries as **hard** drift
...
1. **`CLAIMS.md` restructure** onto the single-paper spine — you chose one paper; the
   argument spine is still in the old shape.
2. **Triage of the 30 `NEEDS_TRIAGE` experiments** against the five pillars — in flight.
3. **`layout.py`'s L1 and L3–L7 have no runnable plants.** Only L2 does. Recorded in
   `CLAUDE.md`; converting them is outstanding work, not a completed claim.
4. **The G4 gate remedy**, to whatever extent it can be reconstructed without B3.
