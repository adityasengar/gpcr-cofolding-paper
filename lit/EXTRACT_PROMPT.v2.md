...
FIRST: read `SCHEMA.md`. It is version 2. Read the `data_shape` grammar section and
the Changelog carefully — v2 fixed defects that a previous extractor hit, and the
...
10. Leave `why_it_matters` EMPTY. `schema_version` is `v2`. `extracted_on` is
