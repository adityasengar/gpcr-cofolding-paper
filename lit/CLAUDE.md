...
lit/
  ...
  INDEX.md          87 blocks, one per paper, lossy by design         (load always)
  STATUS.md         landed vs planned experiments                     (load for anything draft-facing)
  GAPS.md           802 open items across 79 papers — GENERATED by build_gaps.py,
  ...
  refs.bib          91 entries, 87 extracted; 4 remain bibliography-only
...
## The bibliography sweep, and what it found on its first run

**`validate/bibsweep.py`** extracts every DOI from every held PDF and retrieved text, diffs
against `refs.bib`, and ranks what is missing by **how many of our own papers cite it**. It
prints the denominator and self-tests before reporting. Run it when asking "what prior work are
we missing"; it is on-demand, not part of `corpus_check.sh`, because 766 cited DOIs are absent
**by design** (AlphaFold2, ColabFold, the PDB, PoseBusters and similar infrastructure).

**First run, 2026-09-14: 82 of 83 sources scanned, 766 distinct absent DOIs, 82 cited by ≥2 of
our papers, 26 by ≥3.** A hand sweep of five PDFs had found nine; the mechanical sweep found the
one that mattered most and the hand sweep had missed.

**Four papers were added as a result.** The first is the important one:

- **`delalamo2022sampling`** (eLife 2022, cited by **10** of our other papers). **It falsifies a
  negative this session had been giving the orchestrator**: that no paper reports active-state
  GPCRs from shallow-MSA sampling with no transducer. It reaches **both** states of three GPCRs
  at MSA depths down to 16 sequences. **Four caveats bound it** — templates are ON and carry
  state information at shallow depth, so it is not MSA-only; three GPCRs, one per class, none in
  our panel; state is TM-to-reference, not a predicate; and the authors attribute performance to
  training composition, "many active GPCRs" being in the set — **so if active is the memorised
  default, reaching active is not steering.** Read the note before citing it either way.
- **`chakravarty2024memorization`** (Nat Commun 2024) — **now the strongest C8 support in the
  corpus.** Our other confidence papers report an *absent* relationship; this reports an
  *inverted* one, on 92 fold switchers whose both conformations predate training.
- **`ibrahim2019a100`** (JCIM 2019; **PDF supplied 2026-09-14, now full `v3.2`**) — the A100 universal
  activation index. **A published, operationalised class A predicate that ships as a Python script**,
  which is what `paajanen2026activation` and `khaleq2026hyaline` do not. Two hooks: it splits
  intermediates **2:1 toward active**, and it says **"many active nanobody structures are
  predicted to be weakly active"**, contradicting the nanobody-bound-equals-active convention.
- **`monteirodasilva2024subsampled`** (Nat Commun 2024) — the corpus's only **population-level**
  subsampling result, NMR-validated. **Deliberately partial pass; a full second pass is owed** and
  is recorded in its `unresolved`.

**Still absent and worth a decision** (all cited by ≥3 of our papers): `10.1002/pro.4353`
Chakravarty & Porter 2022 "AlphaFold2 fails to predict protein fold switching" (5 citers);
`10.1101/2025.02.03.636309` "Have protein-ligand cofolding methods moved beyond memorisation?"
(9); `10.1093/bioinformatics/btac202` "Impact of protein conformational diversity on AlphaFold
predictions" (4); `10.1021/acs.jcim.3c01936` "Can Protein Structure Prediction Methods Capture
Alternative Conformations?" (3); `10.1002/prot.26562` AFsample massive sampling (3).

**Known limitation:** the DOI regex catches line-broken DOIs as truncated stubs
(`10.1038/s41586`, `10.1021/acs`). They are visible in the output and should be ignored, not
resolved.

**Three PDFs arrived 2026-09-14 and closed three gaps.** `ibrahim2019a100` (the A100 relay —
**verified, our implementation matches the paper exactly**), `yu2026domainmotion` (was HTML-only;
now page-addressable) and `cheng2026af3cluster` (was abstract-only). **A new page offset:
`ibrahim2019a100` is +3937**, auto-detected by `validate/pageoffset.py` — the eighth.

**⚠ And a new extraction trap, on the A100 PDF specifically.** It is **two-column** and
`pdftotext -layout` **interleaves the columns**, so sentences crossing a column break have no
contiguous form in any extraction. Equation (1) and three other sentences are affected. **I made
three unmarked reconstructions before catching it.** Check for interleaving before quoting from
any two-column PDF, and note that `validate/quotecheck_plaintext.py` only covers blockquoted
spans — inline `*"…"*` quotes are NOT checked by it and must be verified by hand.
...
- **Eight notes have NO PDF and therefore NO page numbers** (`corpus_check.sh` lists them; `eddy2018extrinsictrp`, `mazzoni2000gsctpeptide` and `bondar2017preassembly` joined the original four).
