...
- **input_factor_design**: **New in v3.2. NOT CROSSED, and the two legs live in different models — the same structure as `xing2025purified`.** **MSA**: main arm **none** ("no distance restraints, templates, or MSAs were used", p.18); baseline arm clustered (AF-Cluster/DBSCAN) **and** subsampled (U10, U100, p.15). **templates**: none. **ligand**: **VARIED** (apo arm vs ligand arm). **partner**: varied, but bundled. <br>`crossings:` **MSA x ligand NOT CROSSED** — the MSA-manipulation arm runs on AlphaFold2 while the co-input arms run on AF3/Boltz-2/Chai-1, so the two axes never meet inside one model. **partner x ligand CONFOUNDED** — beta2AR receives "agonist AND heterotrimeric Ga-b-g supplied together" with no agonist-free partner condition (p.18). <br>**Do not describe this paper as MSA-free.** That compression cost this project a correction on 2026-09-10: the U10/U100 and AF-Cluster arms run on all four targets including beta2AR with a receptor-state readout, and a reviewer skimming it will think it pre-empts an MSA-depth claim. The defence is real but one sentence deep.
