...
so-called `YY-lock'\,'' \citep[p.~3]{georgiou2025heterogeneity}, described there
...
independent observations: the YY-lock is reported to ``strengthen the TM5--TM7
packing and stabiliz[e] the outward shift of TM6''
\citep[p.~3]{georgiou2025heterogeneity}, so a structure that satisfies one and
not the other is the case a conjunction is meant to catch.

This conjunction is defined for class~A receptors only. The DRY, NPxxY and PIF
motifs ``are absent in family B receptors,'' which ``implies that the mechanisms
of activation between family B and family A receptors are substantially
different'' \citep{hilger2020gcgr}, so rule~(2) has no class~B analogue.
For the 4 class~B receptors it is replaced by a TM6 kink angle below
...
applicable to an unseen one: at 6.26, ``$\sim$6~\AA{} in rhodopsin;
$\sim$14--18~\AA'' across receptors \citep[p.~24]{georgiou2025heterogeneity}; at
6.42 and 6.36, 18~\AA{} and 17.4~\AA{} in the same
work \citep[pp.~3,~5]{hilger2020gcgr}; and ``around 14~\AA'' with no residue
named \citep[p.~13]{ye2026multistatebias}. On the NPxxY axis we are not aware of
