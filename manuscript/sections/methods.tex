...
labelling cannot then adjudicate that labelling. \textbf{[PI: state the source
of the state annotations on the 80 tier-1 rows.]} This matters because the
construct annotation shipped with the same reference set contradicts the RCSB
record on roughly 40\% of evaluable entries, so the labelling is not assumed
reliable elsewhere in this work.
