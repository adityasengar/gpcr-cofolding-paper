...
activation signature. A prediction is called active above 14.932~\AA.
...
regression predictor. It merges two distinct causes: references that genuinely
fail the predicate, and receptors for which the axis is not measurable at all
(EDNRB, for example, carries Leu at 7.53, so an NPxxY hydroxyl distance is
undefined). The E3 union is never applied, and E3 is irrelevant to raw
